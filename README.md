...
- [x] **Real-time AI chat streaming (SSE)** — `POST /conversations/{id}/messages/stream` yields Server-Sent Events (user_message → token chunks → done); frontend accumulates tokens into a live bubble with a blinking cursor; typing indicator (animated dots) shown before first token arrives; input locked for the full duration; smooth `animate-slide-up` per bubble and auto-scroll on every new token
- [x] **AI smart reply suggestions** — `POST /api/v1/ai/suggestions` calls the existing LLM (gpt-4o-mini) with the user's latest message and returns up to 3 short one-line support replies; frontend displays them as indigo quick-reply pills below the chat input after the AI response completes; clicking a pill sends it immediately as the next user message; loading shimmer shown while fetching; failures are silent and non-blocking
- [ ] **WebSocket infrastructure (future)** — Persistent bidirectional socket for multi-user rooms, presence indicators, and sub-token-level streaming; not yet implemented — the live SSE endpoint fully covers single-user real-time chat
...
- [x] **SaaS monetization UI** — Admin "Plans & Billing" page exposes plan definitions, live usage bars, soft-warning / hard-block states, plan comparison cards (Free / Pro / Team), and upgrade modal; upgrades go through Stripe checkout (stub when `STRIPE_SECRET_KEY` not set)
- [x] **Stripe-ready subscription foundation** — `subscription_status` + `current_period_end` columns on users; `billing_events` audit-log table; `GET /admin/billing/status`, `GET /admin/billing/events`, `POST /admin/billing/checkout-session`; Subscription Status card + Billing History list in Plans & Billing page
- [x] **Customer-facing subscription page** — `/billing` page in the customer portal shows current plan, per-user usage bars, subscription status badge, available plan cards with upgrade buttons, and billing event history
- [x] **Stripe billing integration** — Real Stripe Checkout Sessions (subscription mode); `stripe_customer_id` / `stripe_subscription_id` on User model; webhook handler for `checkout.session.completed`, `customer.subscription.updated`, `customer.subscription.deleted`; both admin and customer billing UIs redirect to Stripe and detect `?checkout=success` / `?checkout=cancel` on return; UI dynamically shows "Secure checkout powered by Stripe" or "Stripe not configured / fallback mode" based on live `monetization_status.stripe_enabled` from the backend; graceful stub mode when `STRIPE_SECRET_KEY` is not set
